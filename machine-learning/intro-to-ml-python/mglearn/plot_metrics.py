...
def plot_decision_threshold():
    ...
    fig, axes = plt.subplots(
        2, 3, figsize=(15, 8), subplot_kw={"xticks": (), "yticks": ()}
    )
    ...
    plot_2d_scores(
        svc, X_train, function="decision_function", alpha=0.7, ax=axes[0, 1], cm=ReBl
    )
    ...
    plot_2d_scores(
        svc, X_train, function="decision_function", alpha=0.7, ax=axes[0, 2], cm=ReBl
    )
    ...
    discrete_scatter(
        X_train[mask, 0], np.zeros(bla), y_train[mask], alpha=0.1, ax=axes[1, 2]
    )
