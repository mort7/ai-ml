...
def _call_classifier_chunked(classifier_pred_or_decide, X):
    ...
    for x_chunk in np.array_split(
        X, np.arange(chunk_size, X.shape[0], chunk_size, dtype=np.int32), axis=0
    ):
...
def plot_2d_classification(
    classifier, X, fill=False, ax=None, eps=None, alpha=1, cm=cm3
):
    # multiclass
    if eps is None:
        eps = X.std() / 2.0
...
def plot_2d_scores(
    classifier, X, ax=None, eps=None, alpha=1, cm="viridis", function=None
):
    # binary with fill
    if eps is None:
        eps = X.std() / 2.0
    ...
    if function is None:
        function = getattr(
            classifier, "decision_function", getattr(classifier, "predict_proba")
        )
    else:
...
def plot_2d_separator(
    ...
    if hasattr(classifier, "decision_function"):
        ...
        decision_values = _call_classifier_chunked(classifier.predict_proba, X_grid)[
            :, 1
        ]
