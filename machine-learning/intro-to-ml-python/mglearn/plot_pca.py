...
def plot_pca_illustration():
    ...
    axes[0].scatter(
        X_blob[:, 0], X_blob[:, 1], c=X_pca[:, 0], linewidths=0, s=60, cmap="viridis"
    )
    ...
    axes[0].arrow(
        pca.mean_[0],
        pca.mean_[1],
        S[0] * pca.components_[0, 0],
        S[0] * pca.components_[0, 1],
        width=0.1,
        head_width=0.3,
        color="k",
    )
    axes[0].arrow(
        pca.mean_[0],
        pca.mean_[1],
        S[1] * pca.components_[1, 0],
        S[1] * pca.components_[1, 1],
        width=0.1,
        head_width=0.3,
        color="k",
    )
    ...
    axes[1].scatter(
        X_pca[:, 0], X_pca[:, 1], c=X_pca[:, 0], linewidths=0, s=60, cmap="viridis"
    )
